...
export default class CreateWorkoutPage extends Component {
  ...
  addExercise = (e) => {
    this.setState((prevState) => ({
      exercises: [
        ...prevState.exercises,
        { liftName: "", weight: "", reps: "", sets: "", order: "" },
      ],
    }));
  };
  ...
  handleSubmit = (e) => {
    ...
    let exercisesWithOrder = exercises.map((exercise, index) => ({
      ...exercise,
      order: index + 1,
    }));
        ...
        <div className="form-wrapper">
          ...
          >
            ...
            {exercises.map((val, index) => {
              let order = index + 1;
              let exerciseId = `exercise-${index}`,
                weightId = `weight-${index}`,
                repId = `reps-${index}`,
                setsId = `sets-${index}`;
              return (
            ...
            <div className="form-buttons">
              <button type="button" onClick={this.addExercise}>
                <i id="add-exercise-icon" className="fas fa-plus-circle"></i>Add
                Exercise
              </button>
              <button type="submit">
                <i id="save-icon" className="far fa-save"></i>Save Workout
              </button>
            </div>
          </form>
        </div>
